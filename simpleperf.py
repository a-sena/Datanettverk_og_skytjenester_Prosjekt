...
#socket() method is used to create client socket
    ...
    #the client receives the acknowledgment from the server and the socket is closed
            ...
            if args.format == "B": #if the user specified the format as Bytes 
                transfer_size_client = amount_of_bytes_sent
            elif args.format == "KB":#if the user specified the format as Kilobytes 
                transfer_size_client = amount_of_bytes_sent / 1000
            else: #If the user has specified the format as Megabytes(MB or M)
                transfer_size_client = amount_of_bytes_sent / 1000000
             #calculate total_size
            rate_client = (transfer_size_client * 8 ) / total_duration_client#calculate bandwidth 
     
            print("-" * 60)
            #print measurements
            print("{0}:{1:<15} 0.0 - {2:.0f}       {3:.0f} {4:<2}      {5:.2f} Mbps".format(clientAddress[0], clientAddress[1], int(total_duration_client), transfer_size_client, args.format,rate_client))
